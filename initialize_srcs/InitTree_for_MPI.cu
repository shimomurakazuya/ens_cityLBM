...
//    std::cout << rank << " : " << nsize << std::endl;
    ...
    tree.copyMPIPutGetInfo(tmp_mpiPutGetInfo);
}
...
set_MPIGetInfo(std::vector<LocalNode>& localNode)
{
    ...
    MPI_Win  win_id_get;
    MPI_Win_create(id_local, sizeof(int)*nsize, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_id_get);
    ...
    MPI_Win_create(offset_local, sizeof(int)*nsize, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_offset_get);
