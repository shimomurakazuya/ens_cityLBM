    ...
    )
{
    ...
    // calculate flag //
    if (rank == 0) { std::cout << "calculate flag" << std::endl; }
    id_tmp      = 0;
    for (int lv=0; lv<lv_max; lv++) {
        for (int k=0; k<nz[lv]; k++) {
            for (int j=0; j<ny[lv]; j++) {
    ...
    // allocate flag //
    if (rank == 0) { std::cout << "allocate flag" << std::endl; }
    id_tmp      = 0;
    for (int lv=0; lv<lv_max; lv++) {
        for (int k=0; k<nz[lv]; k++) {
            for (int j=0; j<ny[lv]; j++) {
...
//                    bool  cal_flag = id_init[id_tmp].flag_cal;
    ...
    if (rank == 0) { std::cout << "id_init" << std::endl; }
    id_tmp      = 0;
    id_filtered = 0;
    for (int lv=0; lv<lv_max; lv++) {
        for (int k=0; k<nz[lv]; k++) {
    ...
    )
{
    ...
    if (rank == 0) { std::cout << "set_num_nodes_lv" << std::endl; }
