...
bool MPICommunication::ValCommPackUnpack(
    const int                       lv,
          Field&                    field, 
    ...
    )
{
    ...
    // update //
    val_outdated_[lv] = val_outdated_[lv] + 1;
    if ( !require_mpicomm(lv, val_outdated_[lv]) ) { return false; } // tb


    // MPI Comm //
    std::vector<real*>  vals;
    vals.push_back(valueNS.scalar());
    vals.push_back(valueNS.T());
    ValsComm_pack_unpack(lv, mpiPackUnpackInfo, tree, vals, valueBuff);


    // clear //
    clear_outdated(val_outdated_[lv]);
    ...
    )
{
    ...
    // update //
    lbm_outdated_[lv] = lbm_outdated_[lv] + 1;
...
//    LBMComm_pack_unpack_host(lv, field, mpiPackUnpackInfo, tree, valueLBM, valueBuff);
...
{
    real* sbuff  = valueBuff.sbuff_lbm();
    real* rbuff  = valueBuff.rbuff_lbm();
...
//    PackUnpack::pack<real>(slist, val, sbuff, num_slist);
    if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
        PackUnpack::pack_device<real>(slist, val, sbuff, num_slist);
    }
    else if (valueBuff.memType() == MemType::Host) {
        PackUnpack::pack_host<real>(slist, val, sbuff, num_slist);
    }
    ...
    for (int i=0; i<num_rank_recv; i++) {
        ...
        MPI_Irecv( &rbuff[offset_recv], num_recv, MFLOAT, rank_recv, tag_recv, MPI_COMM_WORLD, &request[i]);
    }
    ...
    for (int i=0; i<num_rank_send; i++) {
        ...
        MPI_Isend( &sbuff[offset_send], num_send, MFLOAT, rank_send, tag_send, MPI_COMM_WORLD, &request[num_rank_recv + i]);
    }
...
//    PackUnpack::unpack<real>(rlist, val, rbuff, num_rlist);
    if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
        PackUnpack::unpack_device<real>(rlist, val, rbuff, num_rlist);
    }
    else if (valueBuff.memType() == MemType::Host) {
        PackUnpack::unpack_host<real>(rlist, val, rbuff, num_rlist);
    }
}
    ...
    )
{
    ...
    for (auto& val : vals) {
        if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
            PackUnpack::pack_device<real>(slist, val, &sbuff[offset_slist], num_slist);
        }
        else if (valueBuff.memType() == MemType::Host) {
            PackUnpack::pack_host<real>(slist, val, &sbuff[offset_slist], num_slist);
        }
    ...
    for (auto& val : vals) {
        ...
        for (int i=0; i<num_rank_recv; i++) {
            ...
            MPI_Irecv( &rbuff[offset_recv], num_recv, MFLOAT, rank_recv, tag_recv, MPI_COMM_WORLD, &request[irequ]);
            irequ++;
        }
        offset_r += num_rlist;
        ...
        for (int i=0; i<num_rank_send; i++) {
            ...
            MPI_Isend( &sbuff[offset_send], num_send, MFLOAT, rank_send, tag_send, MPI_COMM_WORLD, &request[irequ]);
            irequ++;
        }
        offset_s += num_slist;
    }
    ...
    for (auto& val : vals) {
        if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
            PackUnpack::unpack_device<real>(rlist, val, &rbuff[offset_rlist], num_rlist);
        }
        else if (valueBuff.memType() == MemType::Host) {
            PackUnpack::unpack_host<real>(rlist, val, &rbuff[offset_rlist], num_rlist);
        }
...
//    PackUnpack::pack<real>(slist_lbm, f_lbm, sbuff, num_slist);
    if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
        PackUnpack::pack_device<real>(slist_lbm, f_lbm, sbuff, num_slist);
    }
    else if (valueBuff.memType() == MemType::Host) {
        PackUnpack::pack_host<real>(slist_lbm, f_lbm, sbuff, num_slist);
    }
...
//    PackUnpack::unpack<real>(rlist_lbm, f_lbm, rbuff, num_rlist);
    if (valueBuff.memType() == MemType::Device || valueBuff.memType() == MemType::Managed) {
        PackUnpack::unpack_device<real>(rlist_lbm, f_lbm, rbuff, num_rlist);
    }
    else if (valueBuff.memType() == MemType::Host) {
        PackUnpack::unpack_host<real>(rlist_lbm, f_lbm, rbuff, num_rlist);
    }
...
void MPICommunication::LBMComm_pack_unpack_host(
    const int                       lv,
          Field&                    field, 
    const MPIPackUnpackInfo&        mpiPackUnpackInfo,
    const Tree&                     tree,
...
{
    real* f_lbm = valueLBM.f_lbm();
    real* sbuff_d = valueBuff.sbuff_lbm();
    real* rbuff_d = valueBuff.rbuff_lbm();

    real* sbuff = valueBuff.sbuff_lbm_host();
    real* rbuff = valueBuff.rbuff_lbm_host();
...
#define CHECK_MPI_TIME
    ...
    // packing //
    PackUnpack::pack_device<real>(slist_lbm, f_lbm, sbuff_d, num_slist);
    cudaMemcpy(sbuff, sbuff_d, num_slist*sizeof(real), cudaMemcpyDefault);
    ...
    // unpacking //
    cudaMemcpy(rbuff_d, rbuff, num_rlist*sizeof(real), cudaMemcpyDefault);
    PackUnpack::unpack_device<real>(rlist_lbm, f_lbm, rbuff_d, num_rlist);
    ...
    )
{
    ...
    if (rank_ == 0) { std::cout << std::endl << std::flush; }
    ...
    MPI_Iallreduce(&nleaf_s,  &nleaf_s_max,  1, MPI_INT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[0]);
    MPI_Iallreduce(&nleaf_r,  &nleaf_r_max,  1, MPI_INT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[1]);
    MPI_Iallreduce(&nleaf_s,  &nleaf_s_min,  1, MPI_INT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[2]);
    MPI_Iallreduce(&nleaf_r,  &nleaf_r_min,  1, MPI_INT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[3]);
    MPI_Iallreduce(&nleaf_s,  &nleaf_s_sum,  1, MPI_INT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[4]);
    MPI_Iallreduce(&nleaf_r,  &nleaf_r_sum,  1, MPI_INT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[5]);
    
    MPI_Iallreduce(&msec_total,  &msec_total_max,  1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[6]);
    MPI_Iallreduce(&msec_total,  &msec_total_min,  1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[7]);
    MPI_Iallreduce(&msec_total,  &msec_total_sum,  1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[8]);
    
    MPI_Iallreduce(&msec_mpi,  &msec_mpi_max,  1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[9]);
    MPI_Iallreduce(&msec_mpi,  &msec_mpi_min,  1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[10]);
    MPI_Iallreduce(&msec_mpi,  &msec_mpi_sum,  1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[11]);

    MPI_Iallreduce(&msec_pack,  &msec_pack_max,  1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[12]);
    MPI_Iallreduce(&msec_pack,  &msec_pack_min,  1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[13]);
    MPI_Iallreduce(&msec_pack,  &msec_pack_sum,  1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[14]);

    MPI_Iallreduce(&msec_unpack,  &msec_unpack_max,  1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD, &requ_cout[15]);
    MPI_Iallreduce(&msec_unpack,  &msec_unpack_min,  1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD, &requ_cout[16]);
    MPI_Iallreduce(&msec_unpack,  &msec_unpack_sum,  1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &requ_cout[17]);
