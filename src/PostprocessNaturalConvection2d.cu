...
//    double  _Nu_all  = 0.0;
    for (int l=0; l<id_tasks.size(); l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            if ( FuncObj::is_obj( lv_obs[id0] ) ) { continue; }


            // statistics //
            const double lv3[]       = { lv_obs[Index::id(ids, -1,  0,  0)], lv_obs[Index::id(ids,  0,  0,  0)], lv_obs[Index::id(ids,  1,  0,  0)] };
...
//            const double  Tx = (double)( T[Index::id(ids,  1,  0,  0)] - T[Index::id(ids, -1,  0,  0)] ) / (2.0*dx);
            ...
            const double Tobs3[]     = { T_obs[Index::id(ids, -1,  0,  0)], T_obs[Index::id(ids,  0,  0,  0)], T_obs[Index::id(ids,  1,  0,  0)] };
            const int    bcTypesT3[] = { bcTypes_T[Index::id(ids, -1,  0,  0)], bcTypes_T[Index::id(ids,  0,  0,  0)], bcTypes_T[Index::id(ids,  1,  0,  0)] };

            const double  _T[] = { (FuncObj::is_obj( lv3[0] ) && bcTypesT3[0] == BCTypes::BCDirichlet) ? Tobs3[0] : (T3[0] + T3[1])*0.5,
                                   (FuncObj::is_obj( lv3[2] ) && bcTypesT3[2] == BCTypes::BCDirichlet) ? Tobs3[2] : (T3[2] + T3[1])*0.5  };
...
//            const double  _uT = us*Ts;
            ...
            const double uobs3[]     = { u_obs[Index::id(ids, -1,  0,  0)], u_obs[Index::id(ids,  0,  0,  0)], u_obs[Index::id(ids,  1,  0,  0)] };
            const double  _u[] = { FuncObj::is_obj( lv3[0] ) ? uobs3[0] : (u3[0] + u3[1])*0.5,
                                   FuncObj::is_obj( lv3[2] ) ? uobs3[2] : (u3[2] + u3[1])*0.5  };
