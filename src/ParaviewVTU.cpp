...
#include "zlib_encoder.hpp" // zlib test
...
//    std::cout << __PRETTY_FUNCTION__ << std::endl;
    ...
    const auto nprocs = comm_.col_vector().size();
    const auto iproc0 = comm_.world().rank();
    SetPieceSource(fout, step, nprocs, iproc0);
...
void ParaviewVTU::SetDataArray(std::ofstream& fout, std::string name, VTKDataType type, int number_of_components, BYTE* val, uint64_t total_elem)
{
    ...
    util::ZlibEncoder<uint64_t> zenc;
    zenc.PushArray(val, total_byte);
    zenc.Encode();
    //std::cout << " vtk " << name << " " << 100.*zenc.TotalSize()/total_byte << '%' << " compression" << std::endl;
    ...
    offset_ += zenc.TotalSize();

    const auto&& val_zlib = zenc.GetEncodedData();
    add_appended_data(data_, val_zlib.data(), val_zlib.size());
}
...
void ParaviewVTU::SetPieceSource(std::ofstream& fout, int step, int nprocs, int iproc0)
{
    for (int i=0; i<nprocs; i++) {
        fout << " <Piece Source=\"" << fname_vtu(step, i + iproc0) << "\"/>" << std::endl;
    }
}
