...
//    int    num_grids = 0;
    ...
    for (int l=0; l<n_leaf; l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            const real ua  = initTaylorGreen.ua(x[id], y[id], time)/c_ref;
            const real va  = initTaylorGreen.va(x[id], y[id], time)/c_ref;
            const real dpa = initTaylorGreen.dpa(x[id], y[id], time);
