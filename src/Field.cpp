...
init_field_wo_map() {
    ...
    auto&& map = MapData();
    init_grid(grids_);
    init_tree(tree0_, optionParser_, map);
    init_others();
}
...
read_field_params(const int step)
{
    ...
    // data assimilation //
    read_NudgingCoefAdaptation(rstep, nudgingCoefAdaptation_);
}
...
write_field(const int step)
{
    ...
    // make depended dir //
    const std::string& restart_dir = Foldernames::io_folder + "/restart/" + std::to_string(wstep);
    ...
    // data assimilation //
    write_NudgingCoefAdaptation(wstep, nudgingCoefAdaptation_);
#endif // NO_FIELD_WRITEDAT
}


void  Field::
write_field_vtk(const int step)
{
    ...
    // values //
    write_meshValues_vtk(wstep, grids_io_, tree0_, parameters_io_, meshValues_io_, rank);
#endif // NO_FIELD_WRITEDAT
}
...
write_monitor(const int step)
{
    ...
    const int  time_minutes_start = optionParser().wrf_start();
    const real time_sec    = parameters().time_now();
    const int  time_minutes = int(time_sec/60.0) + time_minutes_start;
    ...
    postprocessmonitor_.OutputMonitorData(wstep, time_minutes, tree0_, parameters_, meshValues_ptr
        , valueTimeAverage1min_
        );
    #else
    postprocessmonitor_.OutputMonitorData(wstep, time_minutes, tree0_, parameters_io_, meshValues_io_
         , valueTimeAverage1min_
        );
    #endif
//    postprocessmonitor_.OutputMonitorData(wstep, time_minutes, tree0_, parameters_, meshValues0_, rank_);
}
...
void Field::
init_tree(Tree& tree, const OptionParser& optionParser, const MapData& map) {
    if (comm_.is_rank0()) { std::cout << __PRETTY_FUNCTION__ << std::endl; }
    tree.init_tree_data(grids_, optionParser, map);
}
...
void  Field:: init_parameters(Parameters& parameters)
{
    if (comm_.is_rank0()) { std::cout << __PRETTY_FUNCTION__ << std::endl; }
...
void  Field::read_NudgingCoefAdaptation(const int step, NudgingCoefAdaptation& nudgingCoefAdaptation)
{
    if (comm_.is_rank0()) { std::cout << __PRETTY_FUNCTION__ << std::endl; }

    nudgingCoefAdaptation.load_NudgingCoefAdaptation(step);
}
...
const
{
    ...
    ioData.writeVTKFile   (grids, tree, parameters, meshValues, VTKOutputScale::Slice_Full, 0,
...
void  Field::write_NudgingCoefAdaptation (const int step, const NudgingCoefAdaptation& nudgingCoefAdaptation) const
{
#if defined(NO_FIELD_WRITEDAT) || defined(NO_IODATA_RESTART)
    if (comm_.is_rank0()) { std::cout << __PRETTY_FUNCTION__ << ": skip @ NO_FIELD_WRITEDAT/NO_IODATA_RESTART" << step << std::endl; }
#else
    if (comm_.is_rank0()) { std::cout << __PRETTY_FUNCTION__ << std::endl; }
if(step % 2 == 0) {
    nudgingCoefAdaptation_.save_NudgingCoefAdaptation(step);
    }
#endif
}
