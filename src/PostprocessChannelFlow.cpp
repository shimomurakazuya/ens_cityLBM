    ...
    )
{
    ...
    for (int l=0; l<(int)id_tasks.size(); l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            const real zcell = meshValues[lv].coordinates().zcell(id);
            if (zcell < z_min_channel_ || zcell >= z_max_channel_) { continue; }

            int  idz;
            real z_obj;
            z_obj_channel_flow(idz, z_obj, zcell, dx, lv_obj[id]);
    ...
    )
{
    ...
    for (int l=0; l<(int)id_tasks.size(); l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            add_rms_values(lv, idz, z_obj, u[id]*c_ref, v[id]*c_ref, w[id]*c_ref, T[id]);
        }
        }
        }
    }
...
void  PostprocessChannelFlow::add_exist_flag(const int lv, const int idz, const real z_obj)
{
...
void  PostprocessChannelFlow::add_mean_values(const int lv, const int idz, const real z_obj, const real u, const real v, const real w, const real T)
{
...
void  PostprocessChannelFlow::add_rms_values(const int lv, const int idz, const real z_obj, const real u, const real v, const real w, const real T)
{
    if ( values_[lv][idz].is_exist ) {
...
void  PostprocessChannelFlow::z_obj_channel_flow(int& idz, real& z_obj, const real z, const real dx, const real lv_obj)
{
    const bool is_bottom_side = (z <= 0.0) ? true : false;
//    const double tmp_nlv_obj = (is_bottom_side) ? z[j] - z_floar:
//                                                  z_roof - z[j];
//    
...
//    z_obj = (idz + 0.5)*dx;
    z_obj = (is_bottom_side) ? -lv_obj : 2.0 + lv_obj;
