...
double  Timer::measure_func(const std::function<void()> func)
{
    start();
    func();
    stop();

    const double  elapsed_time = get_elapsed_time_msec();
...
double  Timer::measure_func(const std::function<void()> func, const std::string  message)
{
    message_ = message;
...
double  Timer::get_total_elapsed_time_msec ()
const
{
    return  total_elapsed_time_;
}
...
double  Timer::cal_elapsed_time_msec (
