...
{
    const real* x     = meshValues[lv].coordinates().x();
    const real* y     = meshValues[lv].coordinates().y();
    ...
    for (int l=0; l<id_tasks.size(); l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            const real xcell = x[id] + 0.5*dx;
            const real ycell = y[id] + 0.5*dx;
            ...
            int  idz;
            real z_obs;
            get_idz_zobs_nc3d(idz, z_obs, zcell, dx, lv_obs[id]);
    ...
    )
{
    ...
    for (int l=0; l<id_tasks.size(); l++) {
        ...
        for (int i=0; i<DefAMR::NX_LEAF; i++) {
            ...
            add_rms_values(lv, idz, z_obs, u[id]*c_ref, v[id]*c_ref, w[id]*c_ref);
        }
        }
        }
    }
...
void  PostprocessNaturalConvection3d::add_exist_flag(const int lv, const int idz, const real z_obs)
{
...
void  PostprocessNaturalConvection3d::add_mean_values(const int lv, const int idz, const real z_obs, const real u, const real v, const real w, const real T)
{
...
void  PostprocessNaturalConvection3d::add_rms_values(const int lv, const int idz, const real z_obs, const real u, const real v, const real w)
{
    if ( values_[lv][idz].is_exist ) {
...
void  PostprocessNaturalConvection3d::get_idz_zobs_nc3d(int& idz, real& z_obs, const real z, const real dx, const real lv_obs)
{
