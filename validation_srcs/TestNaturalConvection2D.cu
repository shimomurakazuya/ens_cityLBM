...
//            const double nlv_obs = lv_tmp; // normalized //
            ...
            const int  xdim = 0;
            const int  ydim = 1;
            if ( lv_obs[j] > -dx && xyz[xdim] < box_min[xdim] + dx ) {
                u_obs[j] = 0.0;
                v_obs[j] = 0.0;
                w_obs[j] = 0.0;
            ...
            if      ( fabs(lv_obs[j]) < 3.0*dx && xyz[xdim] < box_min[xdim] + dx ) { bcTypes_T[j] = BCTypes::BCDirichlet; }
            else if ( fabs(lv_obs[j]) < 3.0*dx && xyz[xdim] > box_max[xdim] - dx ) { bcTypes_T[j] = BCTypes::BCDirichlet; }

            if      ( fabs(lv_obs[j]) < 3.0*dx && xyz[xdim] > box_min[xdim] && xyz[xdim] < box_max[xdim] && xyz[ydim] < box_min[ydim] + 2.0*dx ) { bcTypes_T[j] = BCTypes::BCNeumann; }
            else if ( fabs(lv_obs[j]) < 3.0*dx && xyz[xdim] > box_min[xdim] && xyz[xdim] < box_max[xdim] && xyz[ydim] > box_max[ydim] - 2.0*dx ) { bcTypes_T[j] = BCTypes::BCNeumann; }
