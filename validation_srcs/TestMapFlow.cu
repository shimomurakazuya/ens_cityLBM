...
//        const real   dx = meshValues[lv].coordinates().dx();
        ...
        for (int j=0; j<nn_cell; j++) {
            ...
            const real  u_tmp   = FuncObj::is_fluid(lv_obs[j]) ? FuncMapFlow::inflow_velocity(z[j]) : 0.0; // @ header file //
    ...
    for (int i=0; i<n_leaf; i++) {
        ...
        for (int j=0; j<nn_cell; j++) {
            ...
            if ( fabs(roof_lv_obs) <= 2.0*dx ) {
                u_obs[j] = FuncMapFlow::inflow_velocity(z_roof) / c_ref;
                v_obs[j] = 0.0;
                w_obs[j] = 0.0;
            }
        }
...
//    const real dxc = meshValues[0].coordinates().dx();
    ...
    for (int i=0; i<n_leaf; i++) {
        ...
        for (int j=0; j<nn_cell; j++) {
            ...
            if ( FuncObj::is_obj(lv_obs[j]) ) { num_obj++; continue; }
            ...
            if ( xyz[2] > xyz_bc_max[2] - bc_layer_max_[2] ) {
                ...
                u_obs[j] = FuncObj::is_fluid(lv_obs[j]) ? FuncMapFlow::inflow_velocity(xyz[2]) / c_ref : 0.0;
                v_obs[j] = 0.0;
                w_obs[j] = 0.0;

                dirichlet_weight[j] = 0.001;
                viscosity_weight[j] = 0.050;
            }
            // layer region //
...
#pragma omp parallel for
    for (int i=0; i<n_leaf; i++) {
        ...
        for (int j=0; j<nn_cell; j++) {
            ...
            if ( !FuncObj::is_fluid(lv_obs[j]) ) { continue; }
    ...
    )
{
    ...
    for (int i=0; i<n_leaf; i++) {
        ...
        for (int j=0; j<nn_cell; j++) {
            ...
            if ( tmp_lv_obs >= 0.0 && tmp_lv_obs <  0.5*dx ) { tmp_lv_obs =  0.5*dx; }
            if ( tmp_lv_obs <  0.0 && tmp_lv_obs > -0.5*dx ) { tmp_lv_obs = -0.5*dx; }
