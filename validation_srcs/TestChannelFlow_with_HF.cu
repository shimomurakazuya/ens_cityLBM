...
//            const real  u_tmp   = FuncObj::is_fluid(lv_obs[j] + 4.0*dx) ? _u_tmp : 0.0;
//            const real  v_tmp   = FuncObj::is_fluid(lv_obs[j] + 4.0*dx) ? _v_tmp : 0.0;
//            const real  w_tmp   = FuncObj::is_fluid(lv_obs[j] + 4.0*dx) ? _w_tmp : 0.0;
...
//            const double nlv_obs = floor( lv_tmp/dx )*dx + 0.5*dx; // normalized //
            ...
            const double tmp_nlv_obs = (is_bottom_side) ? zcell - z_floar:
                                                          z_roof - zcell;

//            const int    ii = floor(tmp_nlv_obs/dx);
//            const double nlv_obs = (ii + 0.5)*dx;
            const double nlv_obs = floor( tmp_nlv_obs/dx )*dx + 0.5*dx;
