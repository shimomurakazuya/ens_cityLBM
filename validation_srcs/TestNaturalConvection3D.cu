#include "TestNaturalConvection3D.h"
...
void  TestNaturalConvection3D::Flow(int argc, char* argv[])
{
...
void  TestNaturalConvection3D::Init(
    const Grid*         grids,
    const Tree&         tree,
...
{
    constexpr real  box_min[] = { -0.290, -0.290,  0.000 };
    constexpr real  box_max[] = {  0.290,  0.290,  0.800 };
...
void  TestNaturalConvection3D::Init_Value(
    const Grid*         grids,
    const Tree&         tree,
    const Parameters&   parameters,
          MeshValue*    meshValues
...
//            const real  T_tmp   = fluid_property::Temperature0 + (TemperatureHeated_ - fluid_property::Temperature0)*(real)0.15;
...
void  TestNaturalConvection3D::Init_Object(
    const Grid*         grids,
    const Tree&         tree,
    const Parameters&   parameters,
          MeshValue*    meshValues,
...
//            const double nlv_obs = lv_tmp; // normalized //
            const double nlv_obs = floor( lv_tmp/dx )*dx + 0.5*dx; // normalized //
            ...
            v_obs[j] = 0.0;
            w_obs[j] = 0.0;
            T_obs[j] = fluid_property::Temperature0;

            bcTypes_T[j] = BCTypes::BCDirichlet;


            // side wall //
//            T_obs[j] = TemperatureSideWall_;
            T_obs[j] = TemperatureSideWall_ax_*xyz[2] + TemperatureSideWall_b_;


            // heated floar //
            const real  rr = sqrt( xyz[0]*xyz[0] + xyz[1]*xyz[1] );
            if ( rr < 0.250 && xyz[2] > -3.0*dx && xyz[2] < 3.0*dx ) {
                T_obs[j]     = TemperatureHeated_;
                bcTypes_T[j] = BCTypes::BCDirichlet;
            }
            // top wall //
            else if ( xyz[2] > z_max_-3.0*dx && xyz[2] < z_max_+3.0*dx ) {
                T_obs[j]     = TemperatureTopWall_;
//                T_obs[j] = TemperatureSideWall_ax_*xyz[2] + TemperatureSideWall_b_;
                bcTypes_T[j] = BCTypes::BCDirichlet;
            }
            else if ( rr > 0.250 && xyz[2] > -3.0*dx && xyz[2] < 3.0*dx && xyz[0] > -0.290 && xyz[0] < 0.290 && xyz[1] > -0.290 && xyz[1] < 0.290 ) {
                bcTypes_T[j] = BCTypes::BCNeumann;
            }
