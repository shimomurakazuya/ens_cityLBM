...
{
    const real  _f3[] = { (FuncObj::is_obj( lv3[0] ) && bcTypesT3[0] == BCTypes::BCDirichlet) ? Tobj3[0] : f3[0],
                          (FuncObj::is_obj( lv3[1] ) && bcTypesT3[1] == BCTypes::BCDirichlet) ? Tobj3[1] : f3[1],
                          (FuncObj::is_obj( lv3[2] ) && bcTypesT3[2] == BCTypes::BCDirichlet) ? Tobj3[2] : f3[2]  };
...
//                         __f3[2] - __f3[1] };
    ...
    const real  _fx[] = { __f3[1] - __f3[0],
                          __f3[2] - __f3[1] };
    const real  fx[] = { FuncObj::is_obj( lv3[0] ) ? (real)0.0 : _fx[0],
                         FuncObj::is_obj( lv3[2] ) ? (real)0.0 : _fx[1]  };
    ...
    )
{
    ...
    if ( FuncObj::is_obj( lv_obj[id0] ) ) {
        if      ( bcTypes_T[id0] == BCTypes::BCDirichlet ) { T_n[id0] = T_obj[id0];          }
//        else if ( bcTypes_T[id0] == BCTypes::BCRegion    ) { T_n[id0] = T[id0];          }
        else if ( bcTypes_T[id0] == BCTypes::BCRegion    ) { T_n[id0] = f_average( ids, T );          }
        else                                               { T_n[id0] = f_average_w_obj( ids, T, lv_obj ); }
        return;
    }
    ...
    const real  Tobj3x[] = { f_time_weight(T_obj[Index::id(ids, -1,  0,  0)], Tn_obj[Index::id(ids, -1,  0,  0)], time_weight0), f_time_weight(T_obj[Index::id(ids,  0,  0,  0)], Tn_obj[Index::id(ids,  0,  0,  0)], time_weight0), f_time_weight(T_obj[Index::id(ids,  1,  0,  0)], Tn_obj[Index::id(ids,  1,  0,  0)], time_weight0) };
    const real  Tobj3y[] = { f_time_weight(T_obj[Index::id(ids,  0, -1,  0)], Tn_obj[Index::id(ids,  0, -1,  0)], time_weight0), f_time_weight(T_obj[Index::id(ids,  0,  0,  0)], Tn_obj[Index::id(ids,  0,  0,  0)], time_weight0), f_time_weight(T_obj[Index::id(ids,  0,  1,  0)], Tn_obj[Index::id(ids,  0,  1,  0)], time_weight0) };
    const real  Tobj3z[] = { f_time_weight(T_obj[Index::id(ids,  0,  0, -1)], Tn_obj[Index::id(ids,  0,  0, -1)], time_weight0), f_time_weight(T_obj[Index::id(ids,  0,  0,  0)], Tn_obj[Index::id(ids,  0,  0,  0)], time_weight0), f_time_weight(T_obj[Index::id(ids,  0,  0,  1)], Tn_obj[Index::id(ids,  0,  0,  1)], time_weight0) };
...
//
//    const real  dTxx  = (   dfxx(f3x, lv3x, Tobj3x, bcTypesT3x, coef_heatf)
//                          + dfxx(f3y, lv3y, Tobj3y, bcTypesT3y, coef_heatf)
//                          + dfxx(f3z, lv3z, Tobj3z, bcTypesT3z, coef_heatf) );
...
//    const real coef_heatf_total =  coef_heatf / (dx*dx); // because dx of the fxx set to 1 //
    ...
    const real  dTxx  = (   dfxx(f3x, lv3x, Tobj3x, bcTypesT3x, coef_heatf_total)
                          + dfxx(f3y, lv3y, Tobj3y, bcTypesT3y, coef_heatf_total)
                          + dfxx(f3z, lv3z, Tobj3z, bcTypesT3z, coef_heatf_total) );


//    constexpr real  ww   = (real)0.0;
    constexpr real  ww   = (real)0.02;
//    constexpr real  ww   = (real)0.05;
//    constexpr real  ww   = (real)0.20;
//    constexpr real  ww   = (real)0.50;
//    constexpr real  ww   = (real)0.90;
...
inline
__HOST__ __DEVICE__
void  cal_thermal_convection_2nd_bc_heat_flux(
    const int    ids[],
    const real*  T,
          real*  T_n,
    const real*  u,
...
//        else if ( bcTypes_T[id0] == BCTypes::BCRegion    ) { T_n[id0] = T[id0];          }
    ...
    // hflux //
    const real  hflux_zm = (FuncObj::is_obj(lv_obj[Index::id(ids,  0,  0, -1)])) ? hflux_z[ Index::id(ids, 0,  0,  0) ] : (real)0.0;
    const real  hflux_zp = (real)0.0;
...
//    const real coef_heatf_total =  coef_heatf / (dx*dx); // because dx of the fxx set to 1 //
    ...
    const real  dTxx  = (   dfxx_neumann(f3x, lv3x, Tobj3x, bcTypesT3x, coef_heatf_total)
                          + dfxx_neumann(f3y, lv3y, Tobj3y, bcTypesT3y, coef_heatf_total)
                          + dfxx_neumann(f3z, lv3z, Tobj3z, bcTypesT3z, coef_heatf_total) );

    // heat flux //
    const real  dheat_flux = -(real)1.0 * ( (hflux_zp - hflux_zm) ) / dx;


    constexpr real  ww   = (real)0.02;
    const real  dAdv = ((real)1.0-ww)*Tx1st + ww*Tx2nd;
    const real  dDif = (dTxx + dheat_flux) * dt;
    ...
    )
{
    cal_thermal_convection_2nd(
        ...
        sgs_vis,
        lv_obj,
        T_obj, Tn_obj, bcTypes_T, time_weight0,
        coef_heatf,
        dx, dt
        );
}
...
inline
__HOST__ __DEVICE__
void  cal_thermal_convection_bc_heat_flux(
    const int    ids[],
    const real*  T,
          real*  T_n,
    const real*  u,
...
{
    cal_thermal_convection_2nd_bc_heat_flux(
        ids,
        T, T_n,
        u,v,w,
        sgs_vis,
