...
mpirun -x PATH -x LD_LIBRARY_PATH -x PSM2_CUDA=1 -x PSM2_GPUDIRECT=1 -x CUDA_VISIBLE_DEVICES=0,1,2,3 -x HFI_UNIT=0,1,2,3 \
    ...
    -npernode 1 -n 8    --bind-to board  ./run.exe \
    -gpu_per_node              4 \
    -time_end                  600.0 \
    -velocity_lbm              1.0  0.1 \
    -number_of_grid_point      50  2 \
    -domain_min                -0.300  -0.300  -0.050 \
    -domain_length              0.600   0.600   0.900 \
    -cfr_steps                 2000  50000 \
    -cfr_flags                 1  1 \
