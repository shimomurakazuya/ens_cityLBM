...
#    num_nodes = int(ncpus) # q_node
    ...
    ncpux = int(ncpus**0.333334)
#    ncpux = int(ncpus**0.5)
#    ncpuy = int(ncpus/ncpux)
   

    # cal #
    velocity_lbm = [1.0, 0.125]
    if arch == 'gpu':
        cfr_steps = [ 5000,  5000 ]
    elif arch == 'cpu':
        cfr_steps = [ 25,  25 ]
...
#        gpu_per_node_cal = int(gpu_per_node/4)
    ...
    domain_min0    = [  -0.300,  -0.300,  -0.010 ]
    domain_length0 = [   0.600,   0.600,   0.820 ]
    
    
    time_end      = time_end0
    domain_min    = [ domain_min0   [0], domain_min0   [1], domain_min0   [2] ]
    domain_length = [ domain_length0[0], domain_length0[1], domain_length0[2] ]
...
#    strings += '#$ -l q_node=' + str(num_nodes) + '\n'
    strings += '#$ -l h_rt=0:06:00\n'
    strings += '#$ -N ws_' + arch + str(ncpus) + '\n'
    strings += '#$ -p -5\n'
    strings += '#$ -v LD_LIBRARY_PATH=/apps/t3/sles12sp2/free/openmpi/2.1.2-cuda-hfi/gnu/lib:/apps/t3/sles12sp2/cuda/8.0.61/lib.375.66:/apps/t3/sles12sp2/cuda/8.0.61/lib64:/apps/t3/sles12sp2/free/hdf5/1.10.1/gnu/lib\n'
    strings += '\n'
    ...
    strings += 'module load openmpi/2.1.2\n'
    strings += '\n'


    strings += 'export OMP_NUM_THREADS=' + str(omp_num_threads) +'\n'
    strings += 'export PSM2_MQ_RNDV_HFI_THRESH=128000\n'
    strings += '\n'
    ...
    # mpirun
    strings += 'mpirun -x PATH -x LD_LIBRARY_PATH -x PSM2_CUDA=1 -x PSM2_GPUDIRECT=1 -x CUDA_VISIBLE_DEVICES=0,1,2,3 -x HFI_UNIT=0,1,2,3'  + ' \\\n'
    strings += '    -x PSM2_MULTIRAIL=2'  + ' \\\n'
    ...
    strings += '    -number_of_grid_point      ' + str(int(number_of_grid_point[0]/2*ncpux)) + ' ' + str(number_of_grid_point[1]) + ' \\\n'
...
if __name__ == '__main__':
    ...
    number_of_grid_point = [ 128, 2 ] # leaf resolution  #
